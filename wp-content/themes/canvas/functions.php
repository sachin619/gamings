...
function tournamentCancel($teamCancel, $wpdb, $Tradetype, $postId) {
    foreach ($teamCancel as $getTeamId): //for loss distirbution
        $resultBetsLoss = $wpdb->get_results("SELECT sum(round(pts*premium)) as pts,uid,tid,mid,team_id FROM wp_bets WHERE $Tradetype='" . $postId . "'  AND team_id=$getTeamId AND mid=0  group by uid");
        ...
        endforeach;
    endforeach;
    $resultTieLoss = $wpdb->get_results("SELECT sum(round(pts*premium)) as pts,uid,tid,mid,team_id FROM wp_bets WHERE tid='" . $postId . "'  AND team_id=0 AND mid=0  group by uid");
